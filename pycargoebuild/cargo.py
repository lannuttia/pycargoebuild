...
class GitCrate(Crate):
    ...
    def _parsed_url(self) -> urllib.parse.ParseResult:
        """Parse the repository URL for host detection."""
        return urllib.parse.urlparse(self.repository)

    def _is_github(self) -> bool:
        """Check if this is a GitHub repository."""
        return self._parsed_url.netloc == "github.com"

    def _is_gitlab(self) -> bool:
        """Check if this is GitLab.com (not self-hosted)."""
        return self._parsed_url.netloc == "gitlab.com"

    def _is_gitlab_selfhosted(self) -> bool:
        """Check if this is self-hosted GitLab."""
        return (
            self._parsed_url.netloc.startswith("gitlab.")
            and self._parsed_url.netloc != "gitlab.com"
        )

    def _is_gitea(self) -> bool:
        """Check if this is a Gitea instance (e.g., Codeberg)."""
        return self._parsed_url.netloc == "codeberg.org"
    ...
    def repo_ext(self) -> str:
        """
        Get file extension for cargo.eclass tarball naming.

        This is only used for the tarball filename format, not for
        actual fetching (which is done by dulwich).
        """
        if self._is_github():
            return ".gh"
        if self._is_gitlab():
    ...
    def get_workspace_toml(self, distdir: Path) -> dict:
        ...
        with tarfile.open(distdir / filename, "r:gz") as crate_tar:
            ...
            if tarf is None:
                raise RuntimeError(f"{root_dir}/Cargo.toml not found in {filename}")
            with tarf:
            ...
            while (tar_info := crate_tar.next()) is not None:
                ...
                if path.name == "Cargo.toml":
                    ...
                    try:
                        ...
                        continue
                    if metadata.name == self.name and metadata.version == self.version:
                        return path.parent

        raise RuntimeError(
            f"Package {self.name} not found in crate " f"{distdir / self.filename}"
        )

    def get_git_crate_entry(self, distdir: Path) -> str:
        subdir = str(self.get_package_directory(distdir)).replace(
            self.commit, "%commit%"
        )

        if self._is_gitea():
            # Gitea instances need the ;gitea suffix
            return f"{self.repository};{self.commit};{subdir};gitea"
        # All other hosts (GitHub, GitLab, self-hosted GitLab, etc.)
        # use the standard git URL format
...
class PackageMetadata(typing.NamedTuple):
    ...
    def with_replaced_license(self, new_license: str | None) -> "PackageMetadata":
...
def get_crates(f: typing.BinaryIO) -> typing.Generator[Crate, None, None]:
    ...
    for p in cargo_lock["package"]:
        ...
        try:
            if p["source"] == CRATE_REGISTRY:
                if lock_version is None:
                    checksum = cargo_lock["metadata"][
                        f"checksum {p['name']} {p['version']} " f"({p['source']})"
                    ]
                else:
                    checksum = p["checksum"]
                yield FileCrate(name=p["name"], version=p["version"], checksum=checksum)
                ...
                if repo.count("/") != 1:
                    raise RuntimeError("Invalid GitHub/GitLab URL: " f"{p['source']}")
                yield GitCrate(
                    name=p["name"],
                    version=p["version"],
            ...
            raise RuntimeError(
                "Incorrect/insufficient metadata for crate: " f"{p!r}"
            ) from e


def get_meta_key(key: str, pkg_meta: dict, workspace_pkg_meta: dict) -> str | None:
...
def get_package_metadata(
    ...
    if "package" not in cargo_toml and "workspace" in cargo_toml:
        raise WorkspaceCargoTomlError(cargo_toml["workspace"]["members"])

    pkg_meta = cargo_toml["package"]
    _get_meta_key = functools.partial(
        get_meta_key, pkg_meta=pkg_meta, workspace_pkg_meta=workspace_pkg_meta
    )
